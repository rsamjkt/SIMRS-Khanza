    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.layanan_program_kfr,user.sekrining_hipertensi,user.sekrining_kesehatan_penglihatan,user.catatan_observasi_hemodialisa from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    skrining_kesehatan_gigi_mulut_balita=rs.getBoolean("skrining_kesehatan_gigi_mulut_balita");skrining_anemia=rs.getBoolean("skrining_anemia");layanan_program_kfr=rs.getBoolean("layanan_program_kfr");sekrining_hipertensi=rs.getBoolean("sekrining_hipertensi");sekrining_kesehatan_penglihatan=rs.getBoolean("sekrining_kesehatan_penglihatan");catatan_observasi_hemodialisa=rs.getBoolean("catatan_observasi_hemodialisa");
                    setTampil();
                }       
    ...
    private void setTampil(){
        ...
        if("[M]Skrining Hipertensi".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[M]Skrining Hipertensi",sekrining_hipertensi});
        }
        
        if("[M]Skrining Kesehatan Penglihatan".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[M]Skrining Kesehatan Penglihatan",sekrining_kesehatan_penglihatan});
        }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[M]Skrining Hipertensi".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","sekrining_hipertensi='"+tbUser.getValueAt(i,2).toString()+"'");
            }
            
            if("[M]Skrining Kesehatan Penglihatan".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","sekrining_kesehatan_penglihatan='"+tbUser.getValueAt(i,2).toString()+"'");
            }
